...
train <- read.csv("train.xls")
test <- read.csv("test.xls")
...
sales_price_with_Na_val_random <- which(is.na(combined_set["SalePrice"]))
train_rand <- combined_set[-sales_price_with_Na_val_random,]
test_rand <- combined_set[sales_price_with_Na_val_random,]
train_rand
test_rand

#Splitting the train dataset for random forest

set.seed(42) #So that we can reproduce, we set the random "seed"
f <- train_rand[sample(1:1460),] #and sample(1:1460) just mixes up the (1460) rows
#We designate 4/5 of the set as the "training" set and 1/5 as the "test" set
data_train_rand <- train_rand[1:1168,]
data_test_rand  <- train_rand[1169:1460,]
data_test_rand.y = data_test_rand$SalePrice


rf1 <- randomForest(SalePrice ~ ., data = data_train_rand, ntree=1000 , importance= TRUE)
summary(rf1)


yhat.rf1 <- predict(rf1, data_test_rand)
yhat.rf1

yhat.rf_test <- predict(rf1, test)
yhat.rf_test
